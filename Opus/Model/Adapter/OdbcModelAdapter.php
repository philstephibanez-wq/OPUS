...
final class OdbcModelAdapter
    implements OdbcModelAdapterInterface
{
    ...
    public function tableToModel(
        ...
        if ($columns === []) {
            throw new \RuntimeException(
                'OPUS_MODEL_ODBC_TABLE_COLUMNS_EMPTY: ' . $table
            );
        }
        ...
        foreach ($columns as $column) {
            if (!$column instanceof OdbcColumn) {
                throw new \RuntimeException(
                    'OPUS_MODEL_ODBC_COLUMN_INVALID: ' . $table
                );
            }
    ...
    public function readRecords(
        ...
        $rows = $this->connection->fetchTable(
            $model->tableName(),
            $limit
        );
        $records = [];
        ...
        foreach ($rows as $row) {
            $records[] = new ModelRecord(
                $model,
                $this->normalizeRowForModel($model, $row)
            );
        }
    ...
    public function writeRecords(
        ...
        foreach ($records as $record) {
            $row = $record instanceof ModelRecord
                ? $record->toArray()
                : $record;
            $checked = new ModelRecord(
                $model,
                $this->normalizeRowForModel($model, $row)
            );

            $written += $this->connection->insertRow(
                $model->tableName(),
                $checked->toArray()
            );
        }
    ...
    private function fieldFromColumn(
        OdbcColumn $column
    ): ModelField {
        return new ModelField(
            $column->name(),
            $this->modelTypeFromNativeType(
                $column->nativeType()
            ),
            $column->nullable(),
    ...
    private function modelTypeFromNativeType(
        string $nativeType
    ): string {
    ...
    private function lengthForModel(
        OdbcColumn $column
    ): ?int {
        $modelType = $this->modelTypeFromNativeType(
            $column->nativeType()
        );
     ...
     */
    private function normalizeRowForModel(
        TableModel $model,
        array $row
    ): array {
        $normalized = [];
