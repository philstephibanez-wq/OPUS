...
/**
 * OPUS menu component kept for compatibility.
 *
...
//echo "<br> <font color='white'>MENU ".$this->_class." <pre> ".print_r($routes, true)."</pre></font>";
        ...
        foreach ($routes as $num => $route) {
            $pages = explode('|', $route->target['page']);
            foreach($pages as $m => $page) {
            $path = $route->rule;
//echo "<hr><font color='red'> PAGE $page</font>";
               $page = $this->_i18n->translate($page, 1, 'N', true);
//echo "<br><font color='green'> PAGE translated $page</font>";
...
// echo "<br><font color='yellow'>$num<pre>".print_r($params, true)."</pre></font>";
                list($page, $controller) = explode("_", $nodeId);
                $path = $page."/".$controller."/".$params['urlParms'];
               $url = new OPUS_Url_Url('', $path);
