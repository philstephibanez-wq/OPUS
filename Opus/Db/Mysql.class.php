...
class ASAP_BDD_Mysql {
    ...
    private function trace_query($sql, $script, $line): void {
        if (class_exists('ASAP_Debug')) {
            ASAP_Debug::add($sql, $script, $line, DBCOLOR);
        }
    }
    ...
    public function oops($msg = '', $script = __FILE__, $line = __LINE__) {
        ...
        $errorMsg .= '</table>';
        if (class_exists('ASAP_Debug')) {
            ASAP_Debug::add($errorMsg, __CLASS__ . '::' . __FUNCTION__, __LINE__, 'red');
        }
        throw new ASAP_Exception(strip_tags($msg . ' ' . $this->_error), $this->_errno);
    }
}
