...
final class FullstackApplicationScaffoldPlan implements ScaffoldPlanInterface
{
    ...
    public function entries(): array
    {
        ...
        $entries[] = ScaffoldEntry::file("sites/{$app}/frontend/layouts/public/public.layout.json", $this->json([
            'contract' => 'OPUS_FRONTEND_LAYOUT_V1',
            'id' => 'public',
            'template' => 'public.layout.score',
            'slots' => ['header', 'hero', 'main', 'footer'],
            'nested_layouts_allowed' => true,
        ]));
        ...
        $entries[] = ScaffoldEntry::file("sites/{$app}/frontend/sections/site-header/site-header.section.score", $this->siteHeaderScore());
        ...
        $entries[] = ScaffoldEntry::file("sites/{$app}/frontend/sections/home-hero/home-hero.section.score", $this->homeHeroScore());
        ...
        $entries[] = ScaffoldEntry::file("sites/{$app}/frontend/sections/home-main/home-main.section.score", $this->homeMainScore());
        ...
        $entries[] = ScaffoldEntry::file("sites/{$app}/frontend/sections/site-footer/site-footer.section.score", $this->siteFooterScore());
...
<!doctype html>
<html lang="fr">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{{ application.name }}</title><link rel="stylesheet" href="/assets/css/application.css"></head>
<body class="opus-fullstack-application"><header>{{{ slots.header }}}</header><main><section>{{{ slots.hero }}}</section><section id="contract">{{{ slots.main }}}</section></main><footer>{{{ slots.footer }}}</footer></body>
</html>
SCORE;
    }
    ...
    {
        return '<div class="opus-shell opus-header"><strong>{{ application.name }}</strong><nav>[[ foreach: navigation.items as item ]]<a href="{{ item.href }}">{{ item.label }}</a>[[ endforeach ]]</nav></div>';
    }

    private function homeHeroScore(): string
    {
        return '<div class="opus-shell opus-hero"><p>{{ hero.kicker }}</p><h1>{{ hero.title }}</h1><p>{{ hero.subtitle }}</p><a class="opus-button" href="{{ hero.primaryAction.href }}">{{ hero.primaryAction.label }}</a></div>';
    }

    private function homeMainScore(): string
    {
        return '<div class="opus-shell opus-grid">[[ foreach: contractCards as card ]]<article><h2>{{ card.title }}</h2><p>{{ card.text }}</p></article>[[ endforeach ]]</div>';
    }

    private function siteFooterScore(): string
    {
        return '<div class="opus-shell opus-footer"><span>{{ footer.text }}</span></div>';
    }
...
$opusRoot = dirname(dirname(__DIR__));
