 ...
 * OPUS_REFBOOK:
 *   domain: AUTOLOAD
 *   role: Class ClassMapBuilder belongs to the AUTOLOAD Opus framework domain.
 *   contract:
 ...
 *
 * Role:
 *   Build the official Opus classmap cache.
 *
 * Responsibility:
 *   Scan framework/Opus PHP files, extract declared symbols and write a stable
...
final class ClassMapBuilder
{
    ...
    public function build(string $projectRoot, ?string $sourceRoot = null): array
    {
        ...
        $sourceRoot = $sourceRoot === null
            ? $projectRoot . DIRECTORY_SEPARATOR . 'framework' . DIRECTORY_SEPARATOR . 'Opus'
    ...
    public function write(array $map, string $targetFile): void
    {
        if (($map['schema'] ?? '') !== self::SCHEMA) {
            throw new RuntimeException('OPUS_AUTOLOAD_MAP_SCHEMA_INVALID');
        }

        if (($map['duplicates'] ?? []) !== []) {
            throw new RuntimeException('OPUS_AUTOLOAD_DUPLICATE_CLASSES: ' . implode(', ', array_keys((array)$map['duplicates'])));
        }

        $dir = dirname($targetFile);
        if (!is_dir($dir) && !mkdir($dir, 0775, true) && !is_dir($dir)) {
            throw new RuntimeException('OPUS_AUTOLOAD_CACHE_DIR_CREATE_FAILED: ' . $dir);
        }
        ...
        if (file_put_contents($targetFile, $php) === false) {
            throw new RuntimeException('OPUS_AUTOLOAD_CACHE_WRITE_FAILED: ' . $targetFile);
        }
    }
    ...
    private function normalizeRoot(string $path): string
    {
        $real = realpath($path);
        if ($real === false) {
            throw new RuntimeException('OPUS_AUTOLOAD_PATH_NOT_FOUND: ' . $path);
        }
    ...
    private function relativePath(string $root, string $file): string
    {
        ...
        if (!str_starts_with($file, $root)) {
            throw new RuntimeException('OPUS_AUTOLOAD_FILE_OUTSIDE_ROOT: ' . $file);
        }
