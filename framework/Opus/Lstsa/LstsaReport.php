 ...
 * OPUS_REFBOOK:
 *   domain: LSTSA
 *   role: Class LstsaReport belongs to the LSTSA Opus framework domain.
 *   contract:
...
final class LstsaReport
{
    ...
    public function __construct(
        ...
        foreach ([$this->lstsaId, $this->lstsaVersion, $this->runId, $this->startedAt, $this->configHash] as $value) {
            if (trim($value) === '') {
                throw LstsaException::because('OPUS_Lstsa_REPORT_VALUE_EMPTY');
            }
        }
    }
    ...
    public function addCounter(string $name, int $value): void
    {
        if (!array_key_exists($name, $this->counters)) {
            throw LstsaException::because('OPUS_Lstsa_REPORT_COUNTER_UNKNOWN', $name);
        }

        if ($value < 0) {
            throw LstsaException::because('OPUS_Lstsa_REPORT_COUNTER_NEGATIVE', $name);
        }
    ...
    public function finish(string $status): void
    {
        ...
        if (!in_array($status, $allowed, true)) {
            throw LstsaException::because('OPUS_Lstsa_REPORT_STATUS_INVALID', $status);
        }
    ...
    public function toJson(): string
    {
        ...
        if ($json === false) {
            throw LstsaException::because('OPUS_Lstsa_REPORT_JSON_ENCODE_FAILED');
        }
