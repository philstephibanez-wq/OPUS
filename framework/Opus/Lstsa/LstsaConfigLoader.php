 ...
 * OPUS_REFBOOK:
 *   domain: LSTSA
 *   role: Class LstsaConfigLoader belongs to the LSTSA Opus framework domain.
 *   contract:
...
final class LstsaConfigLoader
{
    ...
    {
        if (!is_file($file)) {
            throw LstsaException::because('OPUS_Lstsa_CONFIG_FILE_MISSING', $file);
        }

        $xml = simplexml_load_file($file);

        if (!$xml instanceof SimpleXMLElement) {
            throw LstsaException::because('OPUS_Lstsa_CONFIG_XML_INVALID', $file);
        }
    ...
    public function fromXml(SimpleXMLElement $xml, string $source = '<memory>'): LstsaDefinition
    {
        if (strtolower($xml->getName()) !== 'lstsa') {
            throw LstsaException::because('OPUS_Lstsa_CONFIG_ROOT_INVALID', $xml->getName());
        }
        ...
        foreach ($load->field as $fieldXml) {
            ...
            if (array_key_exists($field->name, $loadFields)) {
                throw LstsaException::because('OPUS_Lstsa_LOAD_FIELD_DUPLICATE', $field->name);
            }
        ...
        if (!$transform instanceof SimpleXMLElement) {
            throw LstsaException::because('OPUS_Lstsa_TRANSFORM_NODE_MISSING', $source);
        }
        ...
        foreach ($transform->field as $fieldXml) {
            ...
            if (array_key_exists($mapping->target, $mappings)) {
                throw LstsaException::because('OPUS_Lstsa_TARGET_FIELD_DUPLICATE', $mapping->target);
            }
    ...
    private function singleChild(SimpleXMLElement $xml, string $name, string $source): SimpleXMLElement
    {
        ...
        if (!$child instanceof SimpleXMLElement) {
            throw LstsaException::because('OPUS_Lstsa_CONFIG_NODE_MISSING', $source . '#' . $name);
        }
    ...
    private function requiredAttr(SimpleXMLElement $xml, string $name, string $source): string
    {
        ...
        if ($value === '') {
            throw LstsaException::because('OPUS_Lstsa_CONFIG_ATTRIBUTE_MISSING', $source . '@' . $name);
        }
