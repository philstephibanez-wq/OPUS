...
final class OwasysFsmActionHandlers
{
    ...
    public function dispatcher(): FsmActionDispatcher
    {
        return new FsmActionDispatcher([
            ...
            'start_creation_flow' => fn (
                string $action,
                array $transition,
                array $context
            ): bool => $this->startCreationFlow($context),
    ...
    private function setCurrentApp(array $context): array
    {
        ...
        $this->registry()->setCurrent($application, $this->actorId($context));
    ...
    private function clearCurrentApp(array $context): bool
    {
        if ($this->registry instanceof OwasysRegistryModel && is_array($this->session->currentApp())) {
            $this->registry->clear($this->actorId($context));
        }
    ...
    {
        $this->registry()->startCreation($this->actorId($context));
    ...
    /** @param array<string,mixed> $context */
    private function actorId(array $context): string
    {
        $identity = is_array($context['identity'] ?? null)
            ? $context['identity']
            : [];

        return (string) ($identity['subject'] ?? $identity['id'] ?? 'runtime');
    }
}
