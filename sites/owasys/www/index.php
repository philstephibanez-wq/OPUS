...
if (PHP_SAPI === 'cli-server') {
    ...
    if ($requestPath !== '/' && !str_contains($requestPath, "\0")) {
        ...
        if ($publicRoot !== false && $candidate !== false && $candidate !== $frontController) {
            ...
            if (str_starts_with(str_replace('\\', '/', $candidate), $prefix) && is_file($candidate)) { return false; }
        }
    }
...
if (!is_file($autoload)) { http_response_code(500); exit('OWASYS_COMPOSER_AUTOLOAD_MISSING'); }
...
if (!is_array($siteConfig)) { http_response_code(500); exit('OWASYS_SITE_CONFIG_INVALID'); }
...
foreach ($files as $relative) {
    ...
    if (!is_file($file)) { http_response_code(500); exit('OWASYS_RUNTIME_COMPONENT_MISSING:' . $relative); }
    require_once $file;
}
