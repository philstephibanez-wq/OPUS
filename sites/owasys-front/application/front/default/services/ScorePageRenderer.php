...
final class OwasysScorePageRenderer
{
    ...
    public function render(string $bodyTemplate, array $data): string
    {
        ...
        $bodyTemplate = $this->templatePath($bodyTemplate);
        $data['assets'] = $assets;
    ...
    private function templatePath(string $template): string
    {
        ...
        if ($template === ''
            ...
            throw new RuntimeException('OWASYS_SCORE_TEMPLATE_PATH_INVALID');
        }
        if (str_starts_with($template, 'default/')
            || str_starts_with($template, 'modules/')) {
            return $template;
        }
