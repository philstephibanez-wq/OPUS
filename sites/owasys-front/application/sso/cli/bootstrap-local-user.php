...
");exit(1);} $siteRoot=dirname(__DIR__,3);$u=trim((string)($argv[1]??''));$p=(string)($argv[2]??'');$role=trim((string)($argv[3]??'admin'));if($u===''||strlen($p)<10){fwrite(STDERR,"USAGE: php sites/owasys-front/application/sso/cli/bootstrap-local-user.php <username> <password-min-10> [role]
