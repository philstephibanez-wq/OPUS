...
$builderFile = rtrim($root, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'framework' . DIRECTORY_SEPARATOR . 'Asap'
    . DIRECTORY_SEPARATOR . 'Autoload' . DIRECTORY_SEPARATOR . 'ClassMapBuilder.php';
$cacheFile = rtrim($root, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'framework' . DIRECTORY_SEPARATOR . 'Asap'
...
echo 'ASAP_AUTOLOADER_CACHE_FILE=' . $out . PHP_EOL;
echo 'ASAP_AUTOLOADER_CACHE_CLASS_COUNT=' . (string)$map['class_count'] . PHP_EOL;
echo 'ASAP_AUTOLOADER_CACHE_BUILD_OK' . PHP_EOL;
...
if ($assert) {
    ...
    foreach ($required as $class) {
        if (!class_exists($class)) {
            fwrite(STDERR, 'ASAP_AUTOLOADER_CACHE_CLASS_NOT_LOADABLE=' . $class . PHP_EOL);
            exit(1);
        }
    }

    echo 'ASAP_AUTOLOADER_CACHE_ASSERT_OK' . PHP_EOL;
}
