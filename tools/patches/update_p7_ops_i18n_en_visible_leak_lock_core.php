...
if (!is_array($additions)) {
    fwrite(STDERR, 'P7_OPS_ENGLISH_VISIBLE_LEAK_ADDITIONS_INVALID' . PHP_EOL);
    exit(1);
}
...
if (!str_contains($source, 'P7_OPS_I18N_EN_VISIBLE_LEAK_LOCK_CORE')) {
    $source = str_replace(
        'P7_OPS_I18N_VISIBLE_STRINGS_FIX_CORE / completed visible labels',
        'P7_OPS_I18N_EN_VISIBLE_LEAK_LOCK_CORE / English visible text must not leak French / P7_OPS_I18N_VISIBLE_STRINGS_FIX_CORE / completed visible labels',
        $source
    );
}
...
if (!str_contains($readme, 'P7_OPS_I18N_EN_VISIBLE_LEAK_LOCK_CORE')) {
    $readme .= PHP_EOL;
    $readme .= '## P7_OPS_I18N_EN_VISIBLE_LEAK_LOCK_CORE' . PHP_EOL . PHP_EOL;
    $readme .= '- Locks English OPS pages against remaining visible French fragments.' . PHP_EOL;
    $readme .= '- Covers the operations-console sentence, counters, cards, statuses and table labels.' . PHP_EOL;
    $readme .= '- Technical operation identifiers and source/destination path values remain untranslated.' . PHP_EOL;
    $readme .= '- Covered by `tools/smokes/smoke_p7_ops_i18n_en_visible_leak_lock_core.php`.' . PHP_EOL;
}
...
echo 'P7_OPS_I18N_EN_VISIBLE_LEAK_LOCK_CORE_UPDATED' . PHP_EOL;
