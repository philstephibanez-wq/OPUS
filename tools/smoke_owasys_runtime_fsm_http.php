...
PHP;
    ...
    $loginBody = http_build_query([
        ...
        'owasys_username' => 'fsm-http',
        'owasys_password' => $password,
    ]);
    $login = owasys_runtime_fsm_http_request($baseUrl, 'POST', '/login', $loginBody);
    ...
    $selectBody = http_build_query([
        ...
        'owasys_app_id' => 'demo-app',
    ]);
    $select = owasys_runtime_fsm_http_request($baseUrl, 'POST', '/applications', $selectBody, $cookie);
    ...
    if (($structure['status'] ?? 0) !== 200 || !str_contains((string) ($structure['body'] ?? ''), 'YOU ARE WORKING ON')) {
