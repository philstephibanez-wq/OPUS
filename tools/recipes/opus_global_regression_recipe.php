...
/**
 * Opus global anti-regression recipe.
 *
 * Public CLI recipe.
 * Role:
 *   Run the stable Opus smoke/unit commands required after each delivery to
...
$reportRoot = $root . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'reports' . DIRECTORY_SEPARATOR . 'opus_global_regression_recipe';
...
$steps = [
    ...
    ['id' => 'P112Q3F_SMOKE', 'command' => ['php', 'tools/smoke/p112q3f_opus_global_robot_chrome_extension_smoke.php']],
    ['id' => 'P113D1_UNIT', 'command' => ['php', 'tests/Contract/RefBookRestApiContractTest.php']],
    ['id' => 'P113D1_SMOKE', 'command' => ['php', 'tools/smoke/p113d1_opus_refbook_rest_api_smoke.php']],
];
...
$summary = [
    'palier' => 'OPUS_GLOBAL_REGRESSION_RECIPE',
...
$base = $reportRoot . DIRECTORY_SEPARATOR . 'OPUS_GLOBAL_REGRESSION_RECIPE_' . $timestamp;
...
if ($failed) {
    fwrite(STDERR, 'OPUS_GLOBAL_REGRESSION_RECIPE_FAILED' . PHP_EOL);
    exit(1);
}

echo 'OPUS_GLOBAL_REGRESSION_RECIPE_OK' . PHP_EOL;
exit(0);
...
function ensureDirectory(string $path): void
{
    ...
    if (!mkdir($path, 0775, true) && !is_dir($path)) { fwrite(STDERR, 'OPUS_GLOBAL_REGRESSION_RECIPE_DIRECTORY_FAILED: ' . $path . PHP_EOL); exit(1); }
}
...
function writeJson(string $path, array $data): void
{
    ...
    if (!is_string($json)) { fwrite(STDERR, 'OPUS_GLOBAL_REGRESSION_RECIPE_JSON_FAILED: ' . $path . PHP_EOL); exit(1); }
    writeText($path, $json . PHP_EOL);
}

function writeText(string $path, string $content): void
{
    if (file_put_contents($path, $content) === false) { fwrite(STDERR, 'OPUS_GLOBAL_REGRESSION_RECIPE_WRITE_FAILED: ' . $path . PHP_EOL); exit(1); }
}

/** @param array<string,mixed> $report */
function buildMarkdown(array $report): string
{
    $lines = ['# Opus Global Regression Recipe', '', 'Status: **' . $report['summary']['status'] . '**', '', '## Steps', ''];
    foreach ($report['steps'] as $step) { $lines[] = '- ' . $step['status'] . ' â€” ' . $step['id'] . ' â€” ExitCode=' . (string) $step['exit_code']; }
...
{
    return '<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>Opus Global Regression Recipe</title></head><body><pre>'
