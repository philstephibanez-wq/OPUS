...
$htmlReport = $reportDir . '/p116b5_live_score_recipe.html';
$emailHtmlReport = $reportDir . '/p116b5_live_score_recipe_email.html';
$jsonReport = $reportDir . '/p116b5_live_score_recipe.json';
...
function p116b5_run_p112q3b2(string $root): void
{
    ...
    if ($previousMode === false) {
        putenv('OPUS_P112Q3B2_MAIL_MODE');
    } else {
        putenv('OPUS_P112Q3B2_MAIL_MODE=' . $previousMode);
    }
    if ($previousRequired === false) {
        putenv('OPUS_P112Q3B2_MAIL_REQUIRED');
    } else {
        putenv('OPUS_P112Q3B2_MAIL_REQUIRED=' . $previousRequired);
    }

    p116b5_assert($exitCode === 0, 'P116B5_P112Q3B2_RECIPE_FAILED', 'exit=' . (string)$exitCode . ' stdout=' . trim($stdout) . ' stderr=' . trim($stderr));
}

function p116b5_route_ends_with(string $route, string $suffix): bool
{
    return $route === $suffix || str_ends_with($route, $suffix);
}
...
function p116b5_initial(string $user): string
{
    return match ($user) {
        ...
        'editor' => 'É',
        'admin' => 'A',
        default => mb_strtoupper(mb_substr($user, 0, 1, 'UTF-8'), 'UTF-8'),
    };
}
...
function p116b5_clean_rights(array $row): string
{
    ...
    if ($user === 'guest') {
        return 'lecture publique uniquement';
    }
    if ($user === 'editor') {
        return 'édition autorisée, administration refusée';
    }
    if ($user === 'admin' && str_contains($route, '/admin')) {
...
{
    if (p116b5_route_ends_with($route, '/fr/contact')) {
        return 'Contact public';
    }
    if (p116b5_route_ends_with($route, '/es/editor/form')) {
        return 'Formulario editor';
    }
    if (p116b5_route_ends_with($route, '/en/admin/settings')) {
...
echo 'P116B5_LIVE_SCORE_RECIPE_OK' . PHP_EOL;
