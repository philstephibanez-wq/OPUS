...
if (($manifest['package'] ?? '') !== 'logandplay/opus-lstsar-manager' || ($manifest['application']['slug'] ?? '') !== 'opus-lstsar-manager') {
    $fail('CHECK_LSTSAR_MANAGER_PACKAGE_ID');
}
...
if (($composer['autoload']['psr-4']['OpusLstsarManager\\'] ?? '') !== 'src/' || ($composer['type'] ?? '') !== 'opus-application') {
...
foreach ($requiredRoutes as $name => $expected) {
    if (!isset($routes[$name])) {
        $fail('CHECK_LSTSAR_MANAGER_ROUTE_MISSING', $name);
    }
    ...
    if (($routes[$name]['profiler']['category'] ?? '') !== 'opus.lstsar_manager') {
        $fail('CHECK_LSTSAR_MANAGER_ROUTE_PROFILER', $name);
    }
...
if (($acl['anonymous'] ?? true) !== false || ($acl['denied_by_default'] ?? false) !== true) {
    $fail('CHECK_LSTSAR_MANAGER_ACL_PROTECTED');
}
if (($acl['lstsar_policy']['raw_sql_allowed'] ?? true) !== false || ($acl['lstsar_policy']['ddl_allowed'] ?? true) !== false || ($acl['lstsar_policy']['execute_allowed'] ?? true) !== false || ($acl['lstsar_policy']['mapping_required'] ?? false) !== true) {
...
foreach (['source', 'destination'] as $endpoint) {
    if (($declaration['config'][$endpoint]['driver'] ?? '') !== 'odbc' || empty($declaration['config'][$endpoint]['model'])) {
        $fail('CHECK_LSTSAR_MANAGER_DECLARATION_ENDPOINT', $endpoint);
    }
}
...
$factory = new LstsarManagerViewModelFactory($repository);
...
if (($factory->mappings()['mapping_required'] ?? false) !== true) {
    $fail('CHECK_LSTSAR_MANAGER_MAPPING_VM');
}
if (!isset($factory->rules()['rules']['security'], $factory->rules()['rules']['transform'], $factory->archiveReport()['archive'], $factory->archiveReport()['report'])) {
    $fail('CHECK_LSTSAR_MANAGER_RULES_VM');
}
...
if (($controller->dashboard()['mode'] ?? '') !== 'lstsar_manager') {
