...
/**
 * ASAP RefBook source tag audit.
 *
 * Read-only checker for ASAP_REFBOOK blocks in framework/Asap sources.
 */
...
if (!is_dir($framework)) {
    fwrite(STDERR, 'ASAP_REFBOOK_AUDIT_FRAMEWORK_ROOT_NOT_FOUND=' . $framework . PHP_EOL);
    exit(1);
}

if (!is_dir($reportRoot) && !mkdir($reportRoot, 0775, true) && !is_dir($reportRoot)) {
    fwrite(STDERR, 'ASAP_REFBOOK_AUDIT_REPORT_ROOT_CREATE_FAILED=' . $reportRoot . PHP_EOL);
    exit(1);
}
...
foreach ($files as $file) {
    ...
    if (preg_match_all('/ASAP_REFBOOK:\s*(.*?)END_ASAP_REFBOOK/s', $content, $matches, PREG_SET_ORDER) > 0) {
...
$report = [
    'schema' => 'ASAP_REFBOOK_SOURCE_TAG_AUDIT_V1',
...
echo 'ASAP_REFBOOK_TAG_AUDIT_SYMBOLS=' . count($symbols) . PHP_EOL;
echo 'ASAP_REFBOOK_TAG_AUDIT_ANNOTATED=' . count($annotated) . PHP_EOL;
echo 'ASAP_REFBOOK_TAG_AUDIT_MALFORMED=' . count($malformed) . PHP_EOL;
echo 'ASAP_REFBOOK_TAG_AUDIT_REPORT=' . $reportRoot . PHP_EOL;
...
function markdownReport(array $report): string
{
    ...
    if ($report['annotated'] === []) {
        $lines[] = '_No ASAP_REFBOOK source tags found yet._';
    }
