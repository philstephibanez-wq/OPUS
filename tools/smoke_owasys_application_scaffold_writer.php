...
try {
    ...
    foreach (['config/site.json', 'config/routes.json', 'config/application.fsm.json', 'config/fsm.json', 'application/default', 'application/states/home/views/index.php', 'www/index.php', 'www/asset/themes/starter/css/theme.css'] as $required) {
    ...
    if (file_exists($absoluteRoot . DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR . 'home')) {
        throw new RuntimeException('OWASYS_WRITER_LEGACY_STATE_ROOT_PRESENT');
    }
    ...
    if (($site['application_fsm'] ?? null) !== 'config/application.fsm.json' || ($site['states_root'] ?? null) !== 'application/states' || ($site['dispatch_model'] ?? null) !== 'state-first') {
        throw new RuntimeException('OWASYS_WRITER_SITE_FSM_POINTER_INVALID');
    }

    $routes = json_decode((string) file_get_contents($absoluteRoot . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'routes.json'), true);
    if (!is_array($routes) || ($routes['routes'][0]['state'] ?? null) !== 'home' || !str_contains((string) ($routes['routes'][0]['view'] ?? ''), 'application/states/home/views/index.php')) {
        throw new RuntimeException('OWASYS_WRITER_STATE_ROUTE_INVALID');
    }
    ...
    if (!is_array($fsm) || ($fsm['contract'] ?? null) !== 'OPUS_APPLICATION_FSM_V1' || ($fsm['dispatch_model'] ?? null) !== 'state-first' || empty($fsm['states']) || !isset($fsm['transitions'])) {
        throw new RuntimeException('OWASYS_WRITER_APPLICATION_FSM_INVALID');
    }
    if (($fsm['states'][0]['state'] ?? null) !== 'home' || !str_contains((string) ($fsm['states'][0]['view'] ?? ''), 'application/states/home/views/index.php')) {
        throw new RuntimeException('OWASYS_WRITER_APPLICATION_FSM_STATE_INVALID');
    }
} finally {
